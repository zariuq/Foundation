...
lemma EDisj₂AppendADisj₂Disj₂! : 𝓢 ⊢ ⋁(Γ ++ Δ) ⭤ ⋁Γ ⋎ ⋁Δ := by
  ...
  case hcons.hcons φ ps hps ihp ψ qs hqs ihq =>
    simp_all only [ne_eq, cons_append, append_eq_nil_iff, and_self, not_false_eq_true, disj₂_cons_nonempty,
      reduceCtorEq]
    exact E!_trans (by
      apply EAA!_of_E!_right;
...
lemma CDisj₂!_iff_CADisj₂! : 𝓢 ⊢ φ ➝ ⋁(ψ :: Γ) ↔ 𝓢 ⊢ φ ➝ ψ ⋎ ⋁Γ := by
  ...
  | cons ψ ih => simp only [ne_eq, reduceCtorEq, not_false_eq_true, disj₂_cons_nonempty]
