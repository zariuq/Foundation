...
instance : Logic.Substitution (Hilbert.Minimal Ax) where
  subst {φ} s h := by
    ...
    induction h with
    | @axm _ s' ih        => show Hilbert.Minimal Ax _; simpa using Minimal.axm (s := s' ∘ s) ih;
...
def buildAxioms.instHasJ1 (h : l.contains .J1 := by decide) : (buildAxioms l).HasJ1 where
  p := 0; q := 1;
  mem_J1 := by
...
def buildAxioms.instHasJ2Plus (h : l.contains .J2Plus := by decide) : (buildAxioms l).HasJ2Plus where
  p := 0; q := 1; r := 2;
  mem_J2Plus := by
    left; left; left; left; right;
    simpa using h;

def buildAxioms.instHasJ4 (h : l.contains .J4 := by decide) : (buildAxioms l).HasJ4 where
  p := 0; q := 1;
  mem_J4 := by
    left; left; left; right;
    simpa using h;

def buildAxioms.instHasJ4Plus (h : l.contains .J4Plus := by decide) : (buildAxioms l).HasJ4Plus where
  p := 0; q := 1; r := 2;
  mem_J4Plus := by
    left; left; right;
    simpa using h;
...
def buildAxioms.instHasM (h : l.contains .M := by decide) : (buildAxioms l).HasM where
  p := 0; q := 1; r := 2;
  mem_M := by
    right;
    simpa using h;
