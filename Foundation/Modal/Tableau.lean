...
lemma exists_finset_lindenbaum_index₁ {Γ : Finset (Formula α)} (hΓ : ↑Γ ⊆ ⋃ i, t[i].1): ∃ m, ∀ φ ∈ Γ, φ ∈ t[m].1 := by
...
lemma exists_finset_lindenbaum_index₂ {Δ : Finset (Formula α)} (hΓ : ↑Δ ⊆ ⋃ i, t[i].2) : ∃ n, ∀ φ ∈ Δ, φ ∈ t[n].2 := by
...
private lemma of_mem₂_boxItr : (□^[n]φ ∈ t.1.2) → (∃ t' : MaximalConsistentTableau 𝓢, ((□⁻¹^[n]'t.1.1) ⊆ t'.1.1) ∧ (φ ∈ t'.1.2)) := by
  ...
  obtain ⟨t', ht'⟩ := lindenbaum (𝓢 := 𝓢) (t₀ := ⟨(□⁻¹^[n]'t.1.1), {φ}⟩) $ by
    ...
    . rintro ψ hψ;
      ...
      simp at hΓ;
      simpa [Set.LO.iff_mem_preboxItr] using hΓ $ by simpa;
    . simpa;
  use t';
  constructor;
  . exact ht'.1;
