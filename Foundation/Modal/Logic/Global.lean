...
instance : Entailment.Łukasiewicz (F := Formula α) (S := Logic α × Set (Formula α)) (L, X) where
  mdp ihφψ ihφ := by show GlobalConsequence _ _ _; simpa using GlobalConsequence.mdp ihφψ ihφ;
...
instance : Entailment.Necessitation (F := Formula α) (S := Logic α × Set (Formula α)) (L, X) where
  nec ihφ := by show GlobalConsequence _ _ _; simpa using GlobalConsequence.nec ihφ
