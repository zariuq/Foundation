...
lemma models_Omega1_iff [V ⊧ₘ* 𝗜𝚺₀] : V ⊧ₘ Omega1.omega1 ↔ ∀ x : V, ∃ y, Exponential (‖x‖ₗ^2) y := by
  simp [models_iff, Omega1.omega1, sq]

lemma omega1_of_ISigma1 [V ⊧ₘ* 𝗜𝚺₁] : V ⊧ₘ Omega1.omega1 := models_Omega1_iff.mpr (fun x ↦ Exponential.range_exists (‖x‖ₗ^2))
...
lemma exists_exponential_sq_length (x : V) : ∃ y, Exponential (‖x‖ₗ^2) y :=
  models_Omega1_iff.mp (ModelsTheory.models V Omega1.omega) x

lemma exists_unique_exponential_sq_length (x : V) : ∃! y, Exponential (‖x‖ₗ^2) y := by
  rcases exists_exponential_sq_length x with ⟨y, h⟩
  exact ExistsUnique.intro y h (fun y' h' ↦ h'.uniq h)

lemma smash_exists_unique (x y : V) : ∃! z, Exponential (‖x‖ₗ * ‖y‖ₗ) z := by
  wlog le : x ≤ y
  ...
  have : Exponential (‖x‖ₗ * ‖y‖ₗ) (bexp z (‖x‖ₗ * ‖y‖ₗ)) := exp_bexp_of_lt (a := z) (x := ‖x‖ₗ * ‖y‖ₗ) this
  exact ExistsUnique.intro (bexp z (‖x‖ₗ * ‖y‖ₗ)) this (fun z' H' ↦ H'.uniq this)

instance : Smash V := ⟨fun a b ↦ Classical.choose! (smash_exists_unique a b)⟩

lemma exponential_smash (a b : V) : Exponential (‖a‖ₗ * ‖b‖ₗ) (a ⨳ b) := Classical.choose!_spec (smash_exists_unique a b)

lemma exponential_smash_one (a : V) : Exponential ‖a‖ₗ (a ⨳ 1) := by simpa using exponential_smash a 1
...
instance smash_defined : 𝚺₀-Function₂ (Smash.smash : V → V → V) via smashDef := .mk <| fun v ↦ by
  suffices Exponential (‖v 1‖ₗ * ‖v 2‖ₗ) (v 0) ↔ v 0 = v 1 ⨳ v 2 by
...
@[simp] lemma smash_lt (a b : V) : ‖a‖ₗ * ‖b‖ₗ < a ⨳ b := (exponential_smash a b).lt

lemma length_smash (a b : V) : ‖a ⨳ b‖ₗ = ‖a‖ₗ * ‖b‖ₗ + 1 := (exponential_smash a b).length_eq
...
@[simp] lemma lt_smash_one_right (a : V) : a < a ⨳ 1 := by
  have : Exponential ‖a‖ₗ (a ⨳ 1) := by simpa using (exponential_smash a 1)
...
@[simp] lemma lt_smash_one_righs (a : V) : a ⨳ 1 ≤ 2 * a + 1 := by
  ...
  · exact (le_iff_lt_length_of_exp (exponential_smash a 1)).mpr (by
      suffices ‖a‖ₗ < ‖a * 2 + 1‖ₗ by simpa [mul_comm 2 a]
      have : ‖a * 2 + 1‖ₗ = ‖a‖ₗ + 1 := by
        simpa using length_mul_pow2_add_of_lt pos (show Pow2 2 from by simp) one_lt_two
      simp [this])

lemma lt_smash_iff {a b c : V} : a < b ⨳ c ↔ ‖a‖ₗ ≤ ‖b‖ₗ * ‖c‖ₗ := (exponential_smash b c).lt_iff_len_le

lemma smash_le_iff {a b c : V} : b ⨳ c ≤ a ↔ ‖b‖ₗ * ‖c‖ₗ < ‖a‖ₗ :=
  not_iff_not.mp <| by simp [lt_smash_iff]

lemma lt_smash_one_iff {a b : V} : a < b ⨳ 1 ↔ ‖a‖ₗ ≤ ‖b‖ₗ := by simpa using lt_smash_iff (a := a) (b := b) (c := 1)
...
lemma bexp_eq_smash (a b : V) : bexp (a ⨳ b) (‖a‖ₗ * ‖b‖ₗ) = a ⨳ b := bexp_eq_of_exp (by simp [length_smash]) (exponential_smash a b)

lemma smash_two_mul (a : V) {b} (pos : 0 < b) : a ⨳ (2 * b) = (a ⨳ b) * (a ⨳ 1) := by
  have h₁ : Exponential (‖a‖ₗ * ‖b‖ₗ + ‖a‖ₗ) (a ⨳ (2 * b)) := by
    simpa [length_two_mul_of_pos pos, mul_add] using exponential_smash a (2 * b)
  have h₂ : Exponential (‖a‖ₗ * ‖b‖ₗ + ‖a‖ₗ) (a ⨳ b * a ⨳ 1) := (exponential_smash a b).add_mul (exponential_smash_one a)
  exact h₁.uniq h₂
