...
@[simp] lemma llen_lt_len_smash_len (K : V) : ‖‖K‖ₗ‖ₗ < ‖K ⨳ ‖K‖ₗ‖ₗ := by
  suffices ‖‖K‖ₗ‖ₗ ≤ ‖K‖ₗ * ‖‖K‖ₗ‖ₗ by simpa [length_smash, lt_succ_iff_le]
  rcases zero_le ‖K‖ₗ with (hK | pos)
  · simp [←hK]
  · exact le_mul_of_pos_left pos

lemma mul_len_lt_len_smash {i I L : V} (hi : i ≤ ‖I‖ₗ) : i * ‖L‖ₗ < ‖I ⨳ L‖ₗ := by
  simpa [length_smash, lt_succ_iff_le] using _root_.mul_le_mul_left hi ‖L‖ₗ

lemma mul_len_lt_len_smash' {i K z : V} (hi : i ≤ ‖z‖ₗ) : i * ‖‖K‖ₗ‖ₗ < ‖z ⨳ ‖K‖ₗ‖ₗ := by
  simpa [length_smash, lt_succ_iff_le] using _root_.mul_le_mul_left hi ‖‖K‖ₗ‖ₗ

noncomputable def ext (L S i : V) : V := S / bexp S (i * ‖L‖ₗ) % (L ⨳ 1)

local notation S "{" L "}[" i "]" => ext L S i

lemma ext_eq_zero_of_lt {L S i : V} (h : ‖S‖ₗ ≤ i * ‖L‖ₗ) : S{L}[i] = 0 := by simp [ext, bexp_eq_zero_of_le h]

@[simp] lemma ext_le_self (L S i : V) : S{L}[i] ≤ S := le_trans (mod_le _ _) (by simp)

lemma ext_graph_aux (z S L i : V) : z = S{L}[i] ↔ (‖S‖ₗ ≤ i * ‖L‖ₗ → z = 0) ∧ (i * ‖L‖ₗ < ‖S‖ₗ → ∃ b ≤ S, Exponential (i * ‖L‖ₗ) b ∧ z = S / b % (L ⨳ 1)) := by
  rcases show ‖S‖ₗ ≤ i * ‖L‖ₗ ∨ i * ‖L‖ₗ < ‖S‖ₗ from le_or_gt _ _ with (le | lt)
  · simp [ext_eq_zero_of_lt le, le, not_lt.mpr le]
  · suffices z = S / bexp S (i * ‖L‖ₗ) % L ⨳ 1 ↔ ∃ b ≤ S, Exponential (i * ‖L‖ₗ) b ∧ z = S / b % L ⨳ 1 by
      simpa [lt, not_le.mpr lt, ext]
    have := exp_bexp_of_lt lt
    constructor
    · rintro rfl; exact ⟨bexp S (i * ‖L‖ₗ), by simp, exp_bexp_of_lt lt, rfl⟩
    · rintro ⟨b, _, H, rfl⟩
      rcases H.uniq (exp_bexp_of_lt lt); rfl

lemma ext_graph (z S L i : V) : z = S{L}[i] ↔
    ∃ lS ≤ S, lS = ‖S‖ₗ ∧ ∃ lL ≤ L, lL = ‖L‖ₗ ∧
      (lS ≤ i * lL → z = 0) ∧
      (i * lL < lS →
  ...
  · suffices (∃ b ≤ S, Exponential (i * ‖L‖ₗ) b ∧ z = S / b % L ⨳ 1)
      ↔ ∃ b ≤ S, Exponential (i * ‖L‖ₗ) b ∧ ∃ hL ≤ 2 * L + 1, Exponential ‖L‖ₗ hL ∧ z = S / b % hL
...
lemma ext_zero_eq_self_of_le {L S : V} (h : ‖S‖ₗ ≤ ‖L‖ₗ) : S{L}[0] = S := by
  rcases zero_le S with (rfl | pos)
  · simp [ext]
  · simp [ext]
    have : bexp S 0 = 1 := (exp_bexp_of_lt (show 0 < ‖S‖ₗ from by simp [pos])).zero_uniq
    simp [this, lt_smash_one_iff.mpr h]

lemma ext_eq_of_ge {L S S' i : V} (h : S ≤ S') : S / bexp S' (i * ‖L‖ₗ) % (L ⨳ 1) = S{L}[i] := by
  rcases show i * ‖L‖ₗ < ‖S‖ₗ ∨ ‖S‖ₗ ≤ i * ‖L‖ₗ from lt_or_ge (i * ‖L‖ₗ) ‖S‖ₗ with (lt | le)
  · unfold ext; congr 2; exact bexp_eq_of_lt_length (lt_of_lt_of_le lt $ length_monotone h) lt
  · simp only [ext_eq_zero_of_lt le]
    rcases show i * ‖L‖ₗ < ‖S'‖ₗ ∨ ‖S'‖ₗ ≤ i * ‖L‖ₗ from lt_or_ge (i * ‖L‖ₗ) ‖S'‖ₗ with (lt' | le')
    · have : S < bexp S' (i * ‖L‖ₗ) := ((exp_bexp_of_lt lt').lt_iff_len_le).mpr le
      simp [this]
    · simp [bexp_eq_zero_of_le le']

lemma ext_eq_of_gt {L S S' i : V} (h : i * ‖L‖ₗ < ‖S'‖ₗ) : S / bexp S' (i * ‖L‖ₗ) % (L ⨳ 1) = S{L}[i] := by
  rcases show i * ‖L‖ₗ < ‖S‖ₗ ∨ ‖S‖ₗ ≤ i * ‖L‖ₗ from lt_or_ge (i * ‖L‖ₗ) ‖S‖ₗ with (lt | le)
  · unfold ext; congr 2; exact bexp_eq_of_lt_length h lt
  · simp only [ext_eq_zero_of_lt le]
    have : S < bexp S' (i * ‖L‖ₗ) := ((exp_bexp_of_lt h).lt_iff_len_le).mpr le
    simp [this]

lemma ext_eq_smash_of_le {L S i : V} (h : i ≤ ‖I‖ₗ) : S / bexp (I ⨳ L) (i * ‖L‖ₗ) % (L ⨳ 1) = S{L}[i] :=
  ext_eq_of_gt (mul_len_lt_len_smash h)

lemma ext_add₁_pow2 {L i S₁ S₂ p : V} (pp : Pow2 p) (h : (i + 1) * ‖L‖ₗ < ‖p‖ₗ) :
    (S₁ + S₂ * p){L}[i] = S₁{L}[i] := by
  rcases zero_le S₂ with (rfl | pos₂)
  · simp
  have lt_len : i * ‖L‖ₗ < ‖S₁ + S₂ * p‖ₗ := calc
        i * ‖L‖ₗ ≤ (i + 1) * ‖L‖ₗ := mul_le_mul_right (by simp)
        _       < ‖p‖ₗ           := h
        _       ≤ ‖S₁ + S₂ * p‖ₗ := length_monotone (le_add_left (le_mul_of_pos_left pos₂))
  have : Exponential ((i + 1) * ‖L‖ₗ) (bexp (S₁ + S₂ * p) (i * ‖L‖ₗ) * L ⨳ 1) := by
    simpa [add_mul]
      using Exponential.add_mul (by simp [lt_len]) (by simpa using exponential_smash L 1)
  have : bexp (S₁ + S₂ * p) (i * ‖L‖ₗ) * L ⨳ 1 ∣ p :=
  ...
  calc
    (S₁ + S₂ * p){L}[i] = (S₁ + p * S₂) / bexp (S₁ + S₂ * p) (i * ‖L‖ₗ) % L ⨳ 1         := by simp [ext, mul_comm S₂]
    _                   = (S₁ + bexp (S₁ + S₂ * p) (i * ‖L‖ₗ) * (L ⨳ 1 * q * S₂)) / bexp (S₁ + S₂ * p) (i * ‖L‖ₗ) % L ⨳ 1 := by simp [←mul_assoc, ←hq]
    _                   = (S₁ / bexp (S₁ + S₂ * p) (i * ‖L‖ₗ) + L ⨳ 1 * q * S₂) % L ⨳ 1 := by rw [div_add_mul_self' _ _ (bexp_pos lt_len)]
    _                   = S₁ / bexp (S₁ + S₂ * p) (i * ‖L‖ₗ) % L ⨳ 1                    := by simp [mul_assoc]
    _                   = S₁{L}[i]                                                     := ext_eq_of_ge le_self_add

lemma ext_add₁_bexp {L i j S₁ S₂ : V} (hi : i ≤ ‖I‖ₗ) (hij : j < i) :
    (S₁ + S₂ * bexp (I ⨳ L) (i * ‖L‖ₗ)){L}[j] = S₁{L}[j] :=
...
lemma ext_add₂_bexp {I i j S₁ S₂ : V} (hij : i + j ≤ ‖I‖ₗ) (hS₁ : ‖S₁‖ₗ ≤ i * ‖L‖ₗ) :
    (S₁ + S₂ * bexp (I ⨳ L) (i * ‖L‖ₗ)){L}[i + j] = S₂{L}[j] := by
  have hie : Exponential (i * ‖L‖ₗ) (bexp (I ⨳ L) (i * ‖L‖ₗ)) := exp_bexp_of_lt (mul_len_lt_len_smash $ le_trans le_self_add hij)
  calc  (S₁ + S₂ * bexp (I ⨳ L) (i * ‖L‖ₗ)){L}[i + j]
      = (S₁ + S₂ * bexp (I ⨳ L) (i * ‖L‖ₗ)) / bexp (I ⨳ L) ((i + j) * ‖L‖ₗ) % (L ⨳ 1)                    := by rw [ext_eq_smash_of_le hij]
    _ = (S₁ + S₂ * bexp (I ⨳ L) (i * ‖L‖ₗ)) / bexp (I ⨳ L) (i * ‖L‖ₗ) / bexp (I ⨳ L) (j * ‖L‖ₗ) % (L ⨳ 1) := by
      simp only [add_mul, ← Arithmetic.div_mul]; congr 2; exact bexp_add (by simp [←add_mul, mul_len_lt_len_smash hij])
    _ = S₂ / bexp (I ⨳ L) (j * ‖L‖ₗ) % (L ⨳ 1)                                                          := by
...
noncomputable def append (I L S i X : V) : V := S % bexp (I ⨳ L) (i * ‖L‖ₗ) + X * bexp (I ⨳ L) (i * ‖L‖ₗ)

lemma append_nil (I L S i : V) : append I L S i 0 = S % bexp (I ⨳ L) (i * ‖L‖ₗ) := by simp [append]

lemma len_append (I L S : V) {i X} (hi : i ≤ ‖I‖ₗ) (hX : 0 < X) : ‖append I L S i X‖ₗ = ‖X‖ₗ + i * ‖L‖ₗ := calc
  ‖append I L S i X‖ₗ = ‖X * bexp (I ⨳ L) (i * ‖L‖ₗ) + S % bexp (I ⨳ L) (i * ‖L‖ₗ)‖ₗ := by simp [append, add_comm]
  _                  = ‖X‖ₗ + log (bexp (I ⨳ L) (i * ‖L‖ₗ))                        := length_mul_pow2_add_of_lt hX
                                                                                      (bexp_pow2 $ mul_len_lt_len_smash hi)
                                                                                      (mod_lt _ $ bexp_pos $ mul_len_lt_len_smash hi)
  _                  = ‖X‖ₗ + i * ‖L‖ₗ                                             := by simp [log_bexp (mul_len_lt_len_smash hi)]

lemma append_lt_smash (I L S : V) {i X} (hi : i < ‖I‖ₗ) (hX : ‖X‖ₗ ≤ ‖L‖ₗ) : append I L S i X < I ⨳ L := by
  rcases zero_le X with (rfl | pos)
  · simpa [append_nil]
      using lt_of_lt_of_le (mod_lt _ (bexp_pos $ mul_len_lt_len_smash $ le_of_lt hi)) (by simp)
  · suffices ‖X‖ₗ + i * ‖L‖ₗ ≤ ‖I‖ₗ * ‖L‖ₗ by simpa [lt_smash_iff, len_append I L S (le_of_lt hi) pos]
    calc
      ‖X‖ₗ + i * ‖L‖ₗ ≤ (i + 1) * ‖L‖ₗ := by simp [add_mul, add_comm (i * ‖L‖ₗ), hX]
      _             ≤ ‖I‖ₗ * ‖L‖ₗ     := mul_le_mul_right (succ_le_iff_lt.mpr hi)

lemma append_lt_sq_smash (I L S : V) {i X} (hi : i ≤ ‖I‖ₗ) (hX : ‖X‖ₗ ≤ ‖L‖ₗ) (Ipos : 0 < I) : append I L S i X < (I ⨳ L)^2 := by
  rcases hi with (rfl | hi)
  · calc
      append I L S ‖I‖ₗ X = S % I ⨳ L + X * I ⨳ L := by simp [append, bexp_eq_smash]
...
lemma ext_append_last (I L S : V) {i X} (hi : i ≤ ‖I‖ₗ) (hX : ‖X‖ₗ ≤ ‖L‖ₗ) : (append I L S i X){L}[i] = X := calc
  (append I L S i X){L}[i] = (S % bexp (I ⨳ L) (i * ‖L‖ₗ) + X * bexp (I ⨳ L) (i * ‖L‖ₗ)){L}[i + 0] := by simp [append]
...
lemma ext_append_lt (I L S : V) {i j X} (hi : i ≤ ‖I‖ₗ) (hij : j < i) :
    (append I L S i X){L}[j] = S{L}[j] := calc
  (append I L S i X){L}[j] = (S % bexp (I ⨳ L) (i * ‖L‖ₗ) + X * bexp (I ⨳ L) (i * ‖L‖ₗ)){L}[j] := rfl
  _                        = (S % bexp (I ⨳ L) (i * ‖L‖ₗ)){L}[j]                              := ext_add₁_bexp hi hij
  _                        = (S % bexp (I ⨳ L) (i * ‖L‖ₗ) + (S / bexp (I ⨳ L) (i * ‖L‖ₗ)) * bexp (I ⨳ L) (i * ‖L‖ₗ)){L}[j] := Eq.symm <| ext_add₁_bexp hi hij
...
def IsSeries (U I L A iter T : V) : Prop := ∀ l < iter, Segment U L A (‖I‖ₗ * l) ‖I‖ₗ (T{L}[l]) (T{L}[l + 1])

def Series (U I L A iter n : V) : Prop := ∃ T < U, IsSeries U I L A iter T ∧ T{L}[0] = 0 ∧ T{L}[iter] = n

def SeriesSegment (U I L A k n : V) : Prop := ∃ nₖ ≤ n, Series U I L A (k / ‖I‖ₗ) nₖ ∧ Segment U L A (‖I‖ₗ * (k / ‖I‖ₗ)) (k % ‖I‖ₗ) nₖ n

lemma SeriesSegment.series {U I L A k n : V} (H : SeriesSegment U I L A k n) :
    ∃ T S, IsSeries U I L A (k / ‖I‖ₗ) T ∧ IsSegment L A (‖I‖ₗ * (k / ‖I‖ₗ)) (k % ‖I‖ₗ) S ∧ T{L}[0] = 0 ∧ T{L}[k / ‖I‖ₗ] = S{L}[0] ∧ S{L}[k % ‖I‖ₗ] = n := by
...
lemma Segment.refl (U L A start n : V) (hL : L ⨳ 1 ≤ U) (hn : ‖n‖ₗ ≤ ‖L‖ₗ) : Segment U L A start 0 n n :=
...
lemma IsSeries.le_add {U I L A iter T : V} (H : IsSeries U I L A iter T) : ∀ l ≤ iter, T{L}[l] ≤ T{L}[0] + ‖I‖ₗ * l := by
  intro l
  ...
  case succ l IH =>
    intro h
    have IH : T{L}[l] ≤ T{L}[0] + ‖I‖ₗ * l := IH (le_trans (by simp) h)
    calc
      T{L}[l + 1] ≤ T{L}[l] + ‖I‖ₗ           := (H l (succ_le_iff_lt.mp h)).le_add
      _           ≤ T{L}[0] + ‖I‖ₗ * (l + 1) := by simpa [mul_add, ←add_assoc] using IH

lemma Series.le_add {U I L A iter n : V} (H : Series U I L A iter n) : n ≤ ‖I‖ₗ * iter := by
...
lemma Series.uniq {U I L A iter n₁ n₂ : V} (H₁ : Series U I L A iter n₁) (H₂ : Series U I L A iter n₂) : n₁ = n₂ := by
  ...
  case succ i IH =>
    ...
    have h₁ : Segment U L A (‖I‖ₗ * i) ‖I‖ₗ (T₁{L}[i]) (T₁{L}[i + 1]) := HT₁ i (lt_iff_succ_le.mpr hi)
    have h₂ : Segment U L A (‖I‖ₗ * i) ‖I‖ₗ (T₁{L}[i]) (T₂{L}[i + 1]) := by simpa [IH] using HT₂ i (lt_iff_succ_le.mpr hi)
    exact h₁.uniq h₂
...
lemma SeriesSegment.le {U I L A k n : V} (H : SeriesSegment U I L A k n) :
  ...
  calc
    n ≤ nₖ + k % ‖I‖ₗ              := hS.le_add
    _ ≤ ‖I‖ₗ * (k / ‖I‖ₗ) + k % ‖I‖ₗ := by simpa [mul_comm] using hT.le_add
    _ = k                         := div_add_mod k ‖I‖ₗ
...
lemma Segment.succ (hU : (I ⨳ L)^2 ≤ U) {start intv nₛ nₑ : V} (H : Segment U L A start intv nₛ nₑ) (hintv : intv < ‖I‖ₗ) (hnₛ : ‖nₛ + ‖I‖ₗ‖ₗ ≤ ‖L‖ₗ) :
  ...
  have le_len_L : ‖S{L}[intv] + fbit A (start + intv)‖ₗ ≤ ‖L‖ₗ := calc
    ‖S{L}[intv] + fbit A (start + intv)‖ₗ ≤ ‖S{L}[intv] + 1‖ₗ     := length_monotone <| by simp
    _                                    ≤ ‖S{L}[0] + intv + 1‖ₗ := length_monotone <| by simpa using H.le_add intv (by rfl)
    _                                    ≤ ‖S{L}[0] + ‖I‖ₗ‖ₗ      := length_monotone <| by simp [add_assoc, succ_le_iff_lt, hintv]
    _                                    ≤ ‖L‖ₗ                  := hnₛ
...
lemma Series.succ (hU : (I ⨳ L)^2 ≤ U) (hIL : ‖‖I‖ₗ^2‖ₗ ≤ ‖L‖ₗ)
    {iter n n' : V} (HT : Series U I L A iter n) (HS : Segment U L A (‖I‖ₗ * iter) ‖I‖ₗ n n') (hiter : iter < ‖I‖ₗ) :
    Series U I L A (iter + 1) n' := by
  have Hn : n ≤ ‖I‖ₗ * iter := HT.le_add
  rcases HT with ⟨T, _, HT, Tₛ, rfl⟩
  let T' := append I L T (iter + 1) n'
  have hn'L : ‖n'‖ₗ ≤ ‖L‖ₗ := calc
    ‖n'‖ₗ ≤ ‖T{L}[iter] + ‖I‖ₗ‖ₗ := length_monotone HS.le_add
    _    ≤ ‖(iter + 1) * ‖I‖ₗ‖ₗ := length_monotone <| by simp [add_mul, mul_comm iter, Hn]
    _    ≤ ‖‖I‖ₗ^2‖ₗ            := length_monotone <| by simpa [sq] using mul_le_mul_right (succ_le_iff_lt.mpr hiter)
    _    ≤ ‖L‖ₗ                := hIL
...
lemma SeriesSegment.succ (hU : (I ⨳ L)^2 ≤ U) (hIL : ‖‖I‖ₗ^2‖ₗ ≤ ‖L‖ₗ)
    {k n : V} (hk : k < ‖I‖ₗ^2) (H : SeriesSegment U I L A k n) :
    SeriesSegment U I L A (k + 1) (n + fbit A k) := by
  have hhk : (k + 1)/‖I‖ₗ ≤ ‖I‖ₗ := by simpa using div_monotone (succ_le_iff_lt.mpr hk) ‖I‖ₗ
  have hnk : n ≤ k := H.le
  have Ipos : 0 < I := by simpa using pos_of_gt hk
  rcases H with ⟨nₘ, hnₘn, HT, HS⟩
  have hnₘL : ‖nₘ + ‖I‖ₗ‖ₗ ≤ ‖L‖ₗ := by
    have : k / ‖I‖ₗ < ‖I‖ₗ := div_lt_of_lt_mul (by simpa [sq] using hk)
    calc
      ‖nₘ + ‖I‖ₗ‖ₗ ≤ ‖‖I‖ₗ * (k / ‖I‖ₗ + 1)‖ₗ := length_monotone <| by simp [mul_add, HT.le_add]
      _        ≤ ‖‖I‖ₗ^2‖ₗ                 := length_monotone <| by simpa [sq] using mul_le_mul_left (lt_iff_succ_le.mp this)
      _        ≤ ‖L‖ₗ                     := hIL
  rcases div_mod_succ k ‖I‖ₗ with (⟨hdiv, hmodsucc, hmod⟩ | ⟨hdiv, hmod⟩)
  · have : Segment U L A (‖I‖ₗ * (k / ‖I‖ₗ)) ‖I‖ₗ nₘ (n + fbit A k) := by
      simpa [div_add_mod, hmod] using HS.succ hU (mod_lt _ $ by simp [Ipos]) hnₘL
    have : Series U I L A ((k + 1) / ‖I‖ₗ) (n + fbit A k) := by
    ...
    exact ⟨n + fbit A k, by rfl, this, by
      ...
      · calc
          ...
          _     ≤ U         := hU
      · calc
          ‖n + fbit A k‖ₗ ≤ ‖n + 1‖ₗ := length_monotone <| by simp
          _              ≤ ‖k + 1‖ₗ := length_monotone <| by simp [hnk]
          _              ≤ ‖‖I‖ₗ^2‖ₗ := length_monotone <| succ_le_iff_lt.mpr hk
          _              ≤ ‖L‖ₗ     := hIL⟩
  · have HS' : Segment U L A (‖I‖ₗ * ((k + 1) / ‖I‖ₗ)) ((k + 1) % ‖I‖ₗ) nₘ (n + fbit A k) := by
      simpa [div_add_mod, hdiv, hmod] using HS.succ hU (mod_lt _ $ by simp [Ipos]) hnₘL
    have HT' : Series U I L A ((k + 1) / ‖I‖ₗ) nₘ := by simpa [hdiv] using HT
...
noncomputable def polyI (A : V) : V := bexp (2 * A) (√‖A‖ₗ)

noncomputable def polyL (A : V) : V := ‖polyI A‖ₗ ^ 2

def polyU (A : V) : V := (2 * A + 1) ^ 128

lemma len_polyI {A : V} (pos : 0 < A) : ‖polyI A‖ₗ = √‖A‖ₗ + 1 :=
  len_bexp (show √‖A‖ₗ < ‖2 * A‖ₗ from by simp [length_two_mul_of_pos pos, lt_succ_iff_le])

lemma polyI_le {A : V} (pos : 0 < A) : ‖A‖ₗ < ‖polyI A‖ₗ ^ 2 := by simp [len_polyI pos]
...
lemma bexp_four_mul {a a' x : V} (hx : 4 * x < ‖a‖ₗ) (hx' : x < ‖a'‖ₗ) :
...
lemma polyI_smash_self_polybounded {A : V} (pos : 0 < A) : (polyI A) ⨳ (polyI A) ≤ (2 * A + 1) ^ 4 := calc
  (polyI A) ⨳ (polyI A) = bexp ((polyI A) ⨳ (polyI A)) ((√‖A‖ₗ + 1) ^ 2) := Eq.symm <| by simpa [sq, len_polyI pos] using bexp_eq_smash (polyI A) (polyI A)
  _                     ≤ bexp ((2 * A) ⨳ (2 * A)) ((2 * √‖A‖ₗ) ^ 2)     :=
    (bexp_monotone_le
  ...
  _                     ≤ bexp ((2 * A) ⨳ (2 * A)) (4 * (√‖A‖ₗ) ^ 2)     := by simp [mul_pow, two_pow_two_eq_four]
  _                     = (bexp (A ⨳ 1) ((√‖A‖ₗ) ^ 2)) ^ 4               :=
    bexp_four_mul
...
lemma NuonAux.succ {A k : V} (H : NuonAux A k n) (hk : k ≤ ‖A‖ₗ) : NuonAux A (k + 1) (n + fbit A k) := by
...
lemma NuonAux.exists {k : V} (hk : k ≤ ‖A‖ₗ) : ∃ n, NuonAux A k n := by
...
lemma NuonAux.succ_elim {A k : V} (hk : k ≤ ‖A‖ₗ) (H : NuonAux A (k + 1) n) : ∃ n', n = n' + fbit A k ∧ NuonAux A k n' := by
  rcases NuonAux.exists hk with ⟨n', H'⟩
  rcases H.uniq (H'.succ hk)
  exact ⟨n', rfl, H'⟩

lemma NuonAux.succ_iff {A k : V} (hk : k ≤ ‖A‖ₗ) : NuonAux A (k + 1) (n + fbit A k) ↔ NuonAux A k n := by
  constructor
  · intro H
    rcases NuonAux.exists hk with ⟨n', H'⟩
...
lemma NuonAux.two_mul {k n : V} (hk : k ≤ ‖A‖ₗ) : NuonAux A k n → NuonAux (2 * A) (k + 1) n := by
  revert n hk
  suffices ∀ n ≤ k, k ≤ ‖A‖ₗ → NuonAux A k n → NuonAux (2 * A) (k + 1) n by
    intro n hk H
...
lemma NuonAux.two_mul_add_one {k n : V} (hk : k ≤ ‖A‖ₗ) : NuonAux A k n → NuonAux (2 * A + 1) (k + 1) (n + 1) := by
  revert n hk
  suffices ∀ n ≤ k, k ≤ ‖A‖ₗ → NuonAux A k n → NuonAux (2 * A + 1) (k + 1) (n + 1) by
    intro n hk H
    exact this n H.le hk H
  induction k using ISigma0.succ_induction
  · definability
...
def Nuon (A n : V) : Prop := Nuon.NuonAux A ‖A‖ₗ n
