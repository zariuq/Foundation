...
instance termShift.defined : 𝚺₁-Function₁ termShift (V := V) L via termShiftGraph L := .mk fun v ↦ by
  simpa [termShiftGraph, termShift, Matrix.empty_eq] using construction.result_defined.defined v
...
instance termShiftVec.defined : 𝚺₁-Function₂ termShiftVec (V := V) L via termShiftVecGraph L := .mk fun v ↦ by
  simpa [termShiftVecGraph, termShiftVec, Matrix.empty_eq] using construction.resultVec_defined.defined v
...
instance termBShift.defined : 𝚺₁-Function₁ termBShift (V := V) L via termBShiftGraph L := .mk fun v ↦ by
  simpa [termBShiftGraph, termBShift, Matrix.empty_eq] using construction.result_defined.defined v
...
instance termBShiftVec.defined : 𝚺₁-Function₂ termBShiftVec (V := V) L via termBShiftVecGraph L := .mk fun v ↦ by
  simpa [termBShiftVecGraph, termBShiftVec, Matrix.empty_eq] using construction.resultVec_defined.defined v
...
@[simp] lemma lt_qqAdd_left (x y : V) : x < x ^+ y := by
  simpa [qqAdd] using nth_lt_qqFunc_of_lt (i := 0) (k := 2) (f := (addIndex : V)) (v := ?[x, y]) (by simp)

@[simp] lemma lt_qqAdd_right (x y : V) : y < x ^+ y := by
  simpa [qqAdd] using nth_lt_qqFunc_of_lt (i := 1) (k := 2) (f := (addIndex : V)) (v := ?[x, y]) (by simp)

@[simp] lemma lt_qqMul_left (x y : V) : x < x ^* y := by
  simpa [qqMul] using nth_lt_qqFunc_of_lt (i := 0) (k := 2) (f := (mulIndex : V)) (v := ?[x, y]) (by simp)

@[simp] lemma lt_qqMul_right (x y : V) : y < x ^* y := by
  simpa [qqMul] using nth_lt_qqFunc_of_lt (i := 1) (k := 2) (f := (mulIndex : V)) (v := ?[x, y]) (by simp)

lemma qqFunc_absolute (k f v : ℕ) : ((^func k f v : ℕ) : V) = ^func (k : V) (f : V) (v : V) := by simp [qqFunc, nat_cast_pair]; norm_cast
...
lemma coe_zero_eq : (𝟎 : V) = (^func 0 ⌜(Language.Zero.zero : (ℒₒᵣ).Func 0)⌝ 0) := by
  simp [Arithmetic.zero, qqFuncN_eq_qqFunc, qqFunc, nat_cast_pair]; constructor <;> norm_cast

lemma coe_one_eq : (𝟏 : V) = (^func 0 ⌜(Language.One.one : (ℒₒᵣ).Func 0)⌝ 0) := by
  simp [Arithmetic.one, qqFuncN_eq_qqFunc, qqFunc, nat_cast_pair]; constructor <;> norm_cast
