...
lemma domain_exists [Nonempty M] [M ⊧ₘ* T] : ∃ x : M, π.Dom x := by
  simpa [models_iff, dom_iff] using models_of_provable (M := M) inferInstance π.domain_nonempty
...
def compDirectTranslation (τ : DirectTranslation T₂ L₃) (π : T₁ ⊳ T₂) : DirectTranslation T₁ L₃ where
  ...
  domain_nonempty := by
    have := π.of_provability τ.domain_nonempty
    simpa [exs, bex, DirectInterpretation.translate] using this
