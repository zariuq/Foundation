...
lemma completeness_of_encodable {φ : Sentence L} :
  ...
  ⟨Derivation.completeness_of_encodable (T := T) (Γ := [φ])
    fun _ _ _ hM ↦ ⟨φ, List.mem_of_mem_head? rfl, fun _ ↦ by
      simp only [Empty.eq_elim, Semiformula.eval_emb]
      exact h hM⟩⟩
...
theorem complete {φ : Sentence L} :
  ...
  have : Entailment.Inconsistent (u : Theory L) := by
    rw [←image_u']; simpa [Theory.lMap] using Theory.inconsistent_lMap L.ofSubLanguage this
