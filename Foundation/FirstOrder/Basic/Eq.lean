...
lemma eval_mk {e} {ε} {φ : Semiformula L μ n} :
  ...
  case hall n φ ih =>
    ...
    · intro h a
      ...
      simp only [Matrix.comp_vecCons']
      exact h ⟦a⟧
    · intro h a
      induction' a using Quotient.ind with a
      have := ih.mpr (h a)
      simp only [Matrix.comp_vecCons'] at this
      exact this
  case hex n φ ih =>
    constructor
    · intro ⟨a, h⟩
...
lemma rel_eq (a b : QuotEq L M) : (@Semiformula.Operator.Eq.eq L _).val (M := QuotEq L M) ![a, b] ↔ a = b := by
  ...
  simp only [eqv, Semiformula.Operator.val]
  have := eval_mk (H := H) (e := ![a, b]) (ε := Empty.elim) (φ := Semiformula.Operator.Eq.eq.sentence)
  simp only [Empty.eq_elim, Matrix.comp_vecCons', Matrix.empty_eq] at this ⊢
  exact this
